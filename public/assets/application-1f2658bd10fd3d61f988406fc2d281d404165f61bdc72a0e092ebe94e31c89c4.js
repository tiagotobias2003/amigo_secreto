...
(function() {
  $(document).on('turbolinks:load', function() {
    $('.update_campaign input').bind('blur', function() {
      return $('.update_campaign').submit();
    });
    $('.update_campaign').on('submit', function(e) {
      $.ajax(e.target.action, {
        ...
        success: function(data, text, jqXHR) {
          return Materialize.toast('Campanha atualizada', 4000, 'green');
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return Materialize.toast('Problema na atualização da Campanha', 4000, 'red');
        }
      });
      return false;
    });
    $('.remove_campaign').on('submit', function(e) {
      $.ajax(e.target.action, {
        ...
        data: {},
        success: function(data, text, jqXHR) {
          return $(location).attr('href', '/campaigns');
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return Materialize.toast('Problema na remoção da Campanha', 4000, 'red');
        }
      });
      return false;
    });
    return $('.raffle_campaign').on('submit', function(e) {
      $.ajax(e.target.action, {
        ...
        data: {},
        success: function(data, text, jqXHR) {
          return Materialize.toast('Tudo certo, em breve os participantes receberão um email!', 4000, 'green');
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return Materialize.toast(jqXHR.responseText, 4000, 'red');
        }
      });
      return false;
    });
...
(function() {
    ...
    $('body').on('click', 'a.remove_member', function(e) {
      $.ajax('/members/' + e.currentTarget.id, {
        ...
        success: function(data, text, jqXHR) {
          ...
          return $('#member_' + e.currentTarget.id).remove();
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return Materialize.toast('Problema na remoção de membro', 4000, 'red');
        }
      });
      return false;
    });
    return $('.new_member').on('submit', function(e) {
      $.ajax(e.target.action, {
        ...
        success: function(data, text, jqXHR) {
          ...
          $('#member_name').focus();
          return Materialize.toast('Membro adicionado', 4000, 'green');
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return Materialize.toast('Problema na hora de incluir membro', 4000, 'red');
        }
      });
      return false;
    });
  ...
  insert_member = function(id, name, email) {
    return $('.member_list').append('<div class="member" id="member_' + id + '">' + '<div class="row">' + '<div class="col s12 m5 input-field">' + '<input id="name" type="text" class="validate" value="' + name + '">' + '<label for="name" class="active">Nome</label>' + '</div>' + '<div class="col s12 m5 input-field">' + '<input id="email" type="email" class="validate" value="' + email + '">' + '<label for="email" class="active" data-error="Formato incorreto">Email</label>' + '</div>' + '<div class="col s3 offset-s3 m1 input-field">' + '<i class="material-icons icon">visibility</i>' + '</div>' + '<div class="col s3 m1 input-field">' + '<a href="#" class="remove_member" id="' + id + '">' + '<i class="material-icons icon">delete</i>' + '</a>' + '</div>' + '</div>' + '</div>');
  };
